...
func DeploySubnetContracts(
	...
	clientC, _, authC, rpcClientC, err := initializeClientAndAuth(CChainURL, ctx, pkey)
	if err != nil {
		return err
	}
	...
	if tokenDecimals != 6 {
		return fmt.Errorf("unexpected token decimals after initialization: got %d, want 6", tokenDecimals)
	}
